...
- **`NON ADDITIVE BY` snapshot polarity corrected to match Snowflake (breaking).** The rows are sorted by the non-additive dimensions and the *last* row of that sort is aggregated, so the default (ascending) direction now selects the **latest** snapshot and `DESC` selects the **earliest** — previously the mapping was inverted, and a view ported from Snowflake (or written to Snowflake's documented semantics) silently returned the opposite-end snapshot. **Migration**: a view that wrote `NON ADDITIVE BY (d DESC)` to get the latest snapshot should drop the `DESC` (write `NON ADDITIVE BY (d)`); one that wrote no direction to get the earliest should now add `DESC`. `NULLS` placement is unchanged and is kept as declared (only the direction is reversed internally), so under the default `NULLS LAST` a NULL key still never outranks a real snapshot; declare `NULLS FIRST` to let a NULL key win.
...
- Derived-metric and fact expression inlining no longer corrupts a same-named column on a *different* table: an unqualified metric/fact reference is still inlined, but the same name appearing as the column part of a qualified reference (`x.revenue`) is left untouched instead of being rewritten into invalid SQL (a fact's own qualified form, `alias.name`, is still inlined). Relatedly, the CREATE-time fact and derived-metric validators now share the crate's single identifier-boundary definition, so a non-ASCII character abutting a name (`revenueΩ`) no longer splits it into a spurious reference or falsely flags an aggregate call.
