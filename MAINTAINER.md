...
| `just test-sql` | SQL logic tests (DDL + query round-trips) | Builds the extension, loads it in DuckDB, and runs the `.test` files **enumerated in `test/sql/TEST_LIST`** — the runner takes `--file-list`, not a directory glob, so a file missing from that list is silently skipped. `just check-test-list` makes that a hard error. |
| `just test-sql-prebuilt` | The above, minus the build | Same body as `test-sql` (which is defined as `build` + this), for callers that already have `build/debug/semantic_views.duckdb_extension`. CI's `SQL logic tests` job uses it because it downloads a prebuilt artifact; running `test-sql` there would rebuild what it just downloaded. |
...
The `Coverage (80% minimum)` job in `CodeQuality.yml` runs `cargo llvm-cov nextest` with the **default (bundled) feature set only**. Two consequences worth keeping in mind so the number isn't misread:
...
> **Fuzz oracle design (TECH-DEBT #33):** the two struct-domain targets (`fuzz_render_roundtrip`, `fuzz_sql_expand`) and `fuzz_query_names` use hand-rolled *structural* oracles (converge-once render idempotence; balanced quotes/parens) rather than executing SQL, so they stay fast and DuckDB-free. Their preconditions must cover every fragment interpolated verbatim into the output — the recurring bug class was an *incomplete* precondition, not a wrong approach. The heavier "does DuckDB accept/return the right rows for the expanded SQL" oracle lives in the proptests, which execute against in-memory DuckDB: `tests/differential_proptest.rs` covers the single-table aggregation path (including a randomized pre-aggregation `where_clause`, whose members range over dimensions, `LABELS = (FILTER)` members and a CHAINED fact — a fact whose expression names another fact, EXP-23), `tests/star_schema_proptest.rs` covers the two-table join / fan-trap fence and the per-grain path (its metric set includes a DERIVED metric referencing a base metric by its own-qualified name, EXP-24) (a `ManyToOne` star: a parent-table metric grouped by a *child* dimension must be rejected, while every accepted query — including those carrying the parent metric, computed at its own grain — must match a hand-written oracle), `tests/multi_hop_join_proptest.rs` extends that to a three-table `ManyToOne` chain (`t → u → w`) with a per-grain oracle spanning up to three grains at once — only a dimension below some metric's grain is rejected, and selecting the grandparent dimension without the parent forces the resolver to pull in the intermediate table, `tests/semi_additive_proptest.rs` covers the semi-additive (`NON ADDITIVE BY`) snapshot path against an independent `MAX`/`MIN` + `IS NOT DISTINCT FROM` oracle (randomized `ASC`/`DESC`, duplicate timestamps and NULLs), `tests/window_metric_proptest.rs` covers the window-metric partition path against an independent correlated-subquery oracle (randomized `PARTITION BY EXCLUDING` vs explicit `PARTITION BY`, `SUM`/`COUNT`/`MIN`/`MAX`, NULL partition keys), and `tests/child_grain_proptest.rs` covers the one join direction the other two omit — a base table LEFT JOINed DOWN to a child, where a childless parent survives as a NULL-extended row — checking the whole phantom-row family against an oracle that uses no join at all: constant-argument aggregates in every spelling (`COUNT(1)`, `SUM(1)`, `COUNT('x')`, `COUNT(DISTINCT 1)`, `COUNT(1+0)`, `MIN(1)`), NULL-insensitive arguments (`SUM(COALESCE(...))`), a cross-table fact reference aggregated at child grain, a randomized pre-aggregation `where_clause` whose members reach BOTH sides of the join (including the case where the predicate empties a real parent's child set, making it childless at filter time), and — the only harness that does — a row-level `facts := [...]` request with its own row-level oracle (EXP-21, EXP-25/26/28, PBT-8, PBT-13). Finally, `tests/metamorphic_proptest.rs` is the one harness with **no independently-formulated oracle**: it checks self-consistency between two formulations that must agree, which is precisely what frees its generator to range over cells the oracled harnesses cannot reach. Three properties — (a) a derived metric `f(m1…mk)` queried alone must equal `f` over its components queried separately, over the same dimensions and `where_clause`; (b) a metric's per-group values recombined with its own combiner (`sum` for `SUM`/`COUNT`, `min`/`max` for `MIN`/`MAX`) must reproduce the ungrouped total; (d) inserting a childless base row whose dimension columns clone an existing one must leave every child-grain metric and every child-fact row byte-identical. Its model declares **two named relationships from one source table to one target** (role-playing, PBT-10 — no other harness does), every member and physical table name is drawn from a five-way identifier-style axis including quoted, embedded-quote and non-ASCII spellings (PBT-12), derived chains reach depth 3 over `+`/`-`/`*`/`NULLIF` ratio, and `where_clause` and row-level `facts` requests are generated. Eight anti-vacuity tests assert the generator reaches each axis over a fixed-seed sample — including that the comparisons see non-NULL numbers rather than agreeing about NULLs — so their thresholds shift if a generator changes, by design. Four `#[ignore]`d properties in the same file state invariants that fail today and record TECH-DEBT #69, #70, #71 and #67(4); they state the CORRECT invariant, so each goes green when its entry is fixed. The exact `parse(render(def)) == def` round-trip lives in `tests/roundtrip_proptest.rs`. A full trust-boundary redesign was considered and declined — see TECH-DEBT #33.
...
|----------|---------|--------------|
| **BuildQuick** | Pull requests (skips doc-only changes) | Fast feedback: extension **build** on Linux x86_64 only, via the DuckDB extension-ci-tools reusable workflow. No `push` trigger (runs on PRs + manual dispatch) — `main` gets the full platform matrix from BuildAll. It invokes `make test_release`, but that runs **no tests** here: `linux_amd64` is on upstream's `SKIP_TESTS` list (see the note below), so this workflow is build-only. |
| **BuildAll** | Push to `main` (skips doc-only changes) | Full build across 5 platforms: Linux x86_64/arm64, macOS x86_64/arm64, Windows x86_64. Excludes WASM, musl, mingw variants. **Runs the full sqllogictest suite on `windows_amd64` and `osx_arm64`** (release build, ~30–55 s each). The other three do not test: `linux_amd64` and `linux_arm64` are skipped by upstream, and `osx_amd64` is cross-compiled on an arm64 runner so its binary cannot be executed. |
...
| **IntegrationChecks** | Push to `main` + pull requests (skips doc-only changes) | Four jobs. `Build debug extension` runs `make configure debug` **once** and uploads the binary; the DuckLake CI test and the **sqllogictest suite** (`just test-sql-prebuilt`) `needs:` it and download that artifact instead of rebuilding. The Python integration job builds its own, because every leaf recipe under `just test-integration` declares `build` as a dependency, so its recipe graph re-enters `make debug` regardless. This is the authoritative sqllogictest gate for pull requests. |
...
**Where sqllogictest actually runs, and where `make test_release` silently does nothing.** `BuildQuick` / `BuildAll` delegate to the extension-ci-tools reusable workflow, whose final step is `make test_release`. Whether that runs anything is decided **per platform** by upstream's `SKIP_TESTS` list in `extension-ci-tools/makefiles/c_api_extensions/base.Makefile`:

| platform | tests? | why |
|---|---|---|
| `linux_amd64` | **no** | upstream `SKIP_TESTS=1` — the Ubuntu runners install the `linux_amd64_gcc4` test runner against the wrong glibc wheel (upstream TODO) |
| `linux_arm64` | **no** | built in Docker; `LINUX_CI_IN_DOCKER=1` sets `SKIP_TESTS=1` |
| `osx_amd64` | **no** | cross-compiled on an arm64 runner, so the binary cannot be executed there |
| `osx_arm64` | **yes** | full suite, release build (~30 s) |
| `windows_amd64` | **yes** | full suite, release build (~55 s) |
| musl / mingw / rtools / wasm | **no** | excluded from our matrix, and on upstream's skip list anyway |

Two consequences worth keeping in mind:

1. **`BuildQuick` builds only `linux_amd64`, so it never tests.** Its `make test_release` step prints `Skipping tests..` and exits 0, which reads as a green build. Before #223 that meant **no** sqllogictest ran on pull requests at all — long enough for an incorrect assertion to reach `main` in #217 (an `IDENT-6` case asserting a quoted name is reported unquoted, corrected in #219). The `sqllogictest` job in `IntegrationChecks` is the authoritative PR gate; do **not** "simplify" it away on the assumption that BuildQuick covers it.
2. **Cross-platform sqllogictest is real, but only on `main`,** via BuildAll's Windows and macOS-arm64 legs. A PR therefore gets Linux-only SQL coverage by design (the suite carries no platform-conditional directives — all 111 files are plain `require semantic_views`), and platform-specific breakage surfaces on the `main` push instead.

You can tell the two apart in a job log without reading it all: a real run prints `Running RELEASE tests..`, then one `[n/111] test/sql/….test` line per file, then `111 test files passed (single process)`; a skipped one prints `Skipping tests..` and nothing else. The `ignoring old recipe for target 'test_extension_*_internal'` warnings are **not** a symptom of skipping — they appear in runs that do execute the suite, and simply record this project's Makefile overriding upstream's recipe.
...
> **Gotcha:** `push` path filters are evaluated against the commits a push *introduces* (the `before..after` range), not the net diff. This applies to every `push`-triggered workflow — `BuildAll`, `Docs`, `Fuzz`, and the `main`-push runs of `CodeQuality` / `IntegrationChecks`. It bites hardest for **Fuzz**, the only one still triggered on non-`main` branch pushes: a feature branch **force-pushed on top of already-merged history** can trigger it even for a doc-only net change, because the push range then includes earlier `src/**` commits (e.g. a squash-merge). The `pull_request`-triggered workflows evaluate path filters against the PR's net diff, so pre-merge PR validation is unaffected.
>
> **Fuzz guards against this (#168).** Restarting a branch from `main` between PRs — the normal flow for a sequence of stacked changes — hits the quirk every time, costing 8 × 10 minutes of runner time to re-fuzz code `main` already fuzzed. `Fuzz.yml` therefore runs a cheap `changes` job first: it recomputes the **two-dot** (tree-vs-tree) diff for `before..after` and skips the matrix when nothing under `src`, `fuzz`, or the Cargo manifests actually changed. It **fails open** — a new branch (all-zero `before`), an unavailable before-commit (force-push GC), or a `workflow_dispatch` run all fuzz rather than skip, and so does a guard job that errors outright. That last case is why the matrix gates on `!cancelled() && … != 'false'` rather than `== 'true'`: a failed `needs` job skips its dependents unless the `if:` uses a status-check function, so the intuitive `== 'true'` would silently stop fuzzing whenever the guard broke. Only an explicit `false`, or a cancelled run, skips the matrix. Moving `Fuzz` to a `pull_request` trigger would fix the same waste but forfeit the CI-2 properties above (`issues: write`, no fork-code fuzzing), so the guard lives in the job, not the trigger.
