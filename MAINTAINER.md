...
You can tell the two apart in a job log without reading it all: a real run prints `Running RELEASE tests..` followed by `N tests run, M failed`; a skipped one prints `Skipping tests..` and nothing else. The `ignoring old recipe for target 'test_extension_*_internal'` warnings are **not** a symptom of skipping — they appear in runs that do execute the suite, and simply record this project's Makefile overriding upstream's recipe.
