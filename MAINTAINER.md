...
The `Fuzz.yml` workflow runs all eight targets (10 minutes each) on any push that touches `src/**`, `fuzz/**`, or the Cargo manifests (a path-filtered trigger, so documentation-only pushes skip it). Because GitHub applies that filter to the commits a push introduces rather than its net effect, a `changes` guard job re-checks the net diff and skips the matrix when the two disagree — see the gotcha under [CI Workflows](#ci-workflows). Crash detection works by checking for artifact files (not the fuzzer exit code), so build failures or timeouts do not trigger false positives.
