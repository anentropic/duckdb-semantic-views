...
| **IntegrationChecks** | Push to `main` + pull requests (skips doc-only changes) | Four jobs. `Build debug extension` runs `make configure debug` **once** and uploads the binary; the DuckLake CI test, the Python integration suite (`just test-integration`) and the **sqllogictest suite** (`just test-sql-prebuilt`) then `needs:` it and download that artifact instead of each rebuilding. This is the authoritative sqllogictest gate for pull requests. |
