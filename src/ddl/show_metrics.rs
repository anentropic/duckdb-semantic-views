...
// Phase 65 Plan 05 Batch 3: legacy `ShowMetricsBindData` +
// `ShowMetricsInitData` + `bind_output_columns` + `emit_rows` retired with
// the H2 query_conn allocation. `ShowMetricRow` + `collect_metrics`
// remain because the new `sv_show_semantic_metrics_bind_rust` /
// `_all_bind_rust` dispatchers still call them.
...
// Legacy `ShowSemanticMetricsVTab` + `ShowSemanticMetricsAllVTab`
// (duckdb-rs VTab impls) RETIRED — Phase 65 Plan 05 Batch 3. The C++
// Catalog API paths (`sv_register_show_semantic_metrics` /
// `sv_register_show_semantic_metrics_all`) dispatch via the
// `sv_show_semantic_metrics_bind_rust` / `_all_bind_rust` Rust
// dispatchers above.
