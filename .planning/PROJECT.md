...
**Shipped v0.1.0** with 6,628 LOC Rust across 58 files in 6 days.
**Tech stack:** Rust, duckdb-rs 1.4.4, serde_json, strsim, proptest, cargo-fuzz.
**Architecture:** Extension is a preprocessor — expands semantic view queries into concrete SQL, DuckDB handles all execution. v0.1.0 implements expansion only; pre-aggregation selection is deferred.
**Persistence:** Sidecar file (`<db>.semantic_views`) bridges DuckDB's execution-lock limitation in scalar `invoke`; synced into `_semantic_views_catalog` table on next load. v0.2.0 C++ shim replaces this with `pragma_query_t` (FTS pattern).
...
| Sidecar file for v0.1.0 catalog persistence | DuckDB locks prevent SQL from scalar `invoke`; sidecar (plain JSON file I/O) is the only deadlock-free pure-Rust option | ✓ Accepted (temporary) — works reliably; v0.2.0 replaces with pragma_query_t |
...
| VARCHAR output columns | All semantic_query output columns declared as VARCHAR; avoids type mismatch panics | ⚠️ Revisit — typed output columns would be better UX in v0.2.0 |
