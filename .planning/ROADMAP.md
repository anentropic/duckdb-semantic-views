...
**Plans**:
  ...
  - 65-05 (NEXT): Read-path migration wave — 15 table-function + 2 scalar callbacks migrate to C++ Catalog API (sv_register_table_function from Plan 04); per-call `Connection(*context.db)` from ClientContext; H2 query_conn retires in the final commit.
