...
.. list-table::
   ...
   * - Pre-aggregation ``WHERE`` -- ``SEMANTIC_VIEW( v METRICS ... DIMENSIONS ... WHERE <predicate> )``, where the predicate `may refer only to dimensions, facts, and expressions over them <https://docs.snowflake.com/en/sql-reference/constructs/semantic_view>`_ and "is applied before the metrics are computed"
     - **Partly supported** as the ``where_clause := '...'`` named parameter (``where`` is a reserved word in DuckDB's named-parameter position, so it cannot be spelled ``where :=``). The predicate names declared dimensions and facts, is substituted to their expressions, and is emitted before the ``GROUP BY`` -- so "revenue for orders shipped after X" now recomputes each group over the matching rows. Referencing a metric is rejected, matching Snowflake. **Not yet injected** for queries computed via the per-grain, semi-additive-snapshot, or window strategies: those must filter inside their CTEs, and each raises a clear error rather than applying the filter too late.
