...
### 36. ❓ Multi-grain queries involving window / active semi-additive / role-playing metrics still error

- **Origin:** v0.12.0 (Phase 69), the residual of entry #35. Recorded separately so #35's resolution is not overstated.
- **Decision:** the per-grain planner (`per_grain::is_eligible`) declines — leaving the v0.11.0 fan-trap error in place — when a queried metric is a window metric, an *active* semi-additive metric (a `NON ADDITIVE BY` whose snapshot dimension is outside the query), or carries `USING` role-playing context, and when a queried dimension has no source table (an unqualified expression, whose binding would move if the anchor moved). Each of those strategies emits its own base-anchored CTE (`__sv_snapshot` / `__sv_agg`) or needs role context the grain CTEs do not carry; routing them through per-grain emission without designing that interaction would risk exactly the silent-wrong-answer class v0.11.0 closed.
- **Impact:** Snowflake answers these; we error. In practice they are the corner where multi-grain meets three features that are themselves uncommon — a single-grain query using any of them is unaffected.
- **Action:** extend the per-grain emitter to anchor the semi-additive snapshot and window aggregation CTEs at a metric's own grain, and to thread `USING` context into `anchor_joins`. Each is independently shippable; the eligibility predicate is the single place that decides.
