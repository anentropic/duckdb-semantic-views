...
### 36. ❓ Multi-grain queries involving active semi-additive / role-playing metrics still error (window metrics: RESOLVED)
...
- **Action:** extend the per-grain emitter to anchor the semi-additive snapshot CTE at a metric's own grain, and thread `USING` context into `anchor_joins`. Each is independently shippable; the eligibility predicate is the single place that decides. The window sub-item above is the worked precedent.
