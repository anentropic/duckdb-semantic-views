...
- **Note (rationale updated):** The original entry justified the manual entrypoint by "creating an independent `duckdb_connection` for `semantic_query()`." That is no longer how queries run: since Phase 65 `semantic_view()` (and the read-side TFs) execute on a **fresh per-call `Connection(*context.db)` opened inside the C++ bind callback**, not a long-lived independent connection — there are no `duckdb_connect` calls in `src/`. The manual entrypoint is retained for the database-handle capture the registration path needs.
- **Action:** None needed.
