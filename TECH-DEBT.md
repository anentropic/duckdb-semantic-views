...
### 37. ❓ `JoinTree`'s parent map is degenerate when several tables reference the same parent (fan-in)
...
- **Decision:** `JoinTree::from_graph` takes each node's parent as `graph.reverse[node].first()` — the FIRST table that references it. With fan-in (two children of one parent, e.g. `line_items` and `shipments` both referencing `orders`) `reverse["orders"]` has two entries, so the base table itself is given a "parent" (the first-declared child) and the chain from any node can run *through* the root and out the other side. The fence no longer depends on this: it walks the undirected relationship graph (`expand::fan_trap::GrainGraph`, v0.12.0), which is why the sibling-dimension fan trap it previously missed is now caught. The remaining consumers still walk the parent map.
- **Impact:** `expand::fan_trap::validate_fact_table_path` and the `extension`-gated `SHOW SEMANTIC DIMENSIONS … FOR METRIC` filter ask ancestry questions of this map. Two tables on the same root-to-leaf path but on the *far side of a fan-in* do not see each other as ancestors: a fact query selecting a fact on `shipments` with a dimension on `customers` (`s → o → c`, all many-to-one) is rejected with `FactPathViolation` even though the path is safe. The failure mode is a **false rejection**, not a wrong answer — the pairs it wrongly separates are exactly the ones where no aggregation happens (facts) or where a dimension is merely hidden from a listing.
- **Action:** derive the parent map by BFS from the base table over the undirected edges — the same walk `expand::join_resolver::build_tree_parents` already performs for join emission — so each node's parent is its neighbour *toward* the base table. Then `ancestors_to_root` is correct under fan-in and both consumers are fixed. Needs a fact-path test for the `s → o → c` shape and a `SHOW … FOR METRIC` test for its sibling equivalent.

---

**Last updated:** 2026-07-28 (v0.12.0) — entry #35 RESOLVED: per-grain ("own-grain") metric aggregation landed in `src/expand/per_grain.rs`, so the multi-grain shapes v0.11.0 could only reject are now computed the way Snowflake computes them; the residual (window / active semi-additive / role-playing metrics in a multi-grain query) is entry #36. Prior: 2026-07-20 (v0.11 unreleased) — entry #35 added: the per-grain ("own-grain") metric-aggregation real fix for the fan-trap class is deferred to a follow-up milestone; the shipped EXP-1/2/3 work errors (`RootGrainFanTrap` / `MetricFanTrap`) rather than computing multi-grain queries, and the deferral now has a TECH-DEBT home instead of living only in the 2026-07-02 / 2026-07-18 review notes. Prior same-day: entry #34 resolved: the
