...
- **Action:** extend the per-grain emitter to anchor the semi-additive snapshot CTE at a metric's own grain, and to thread `USING` context into `anchor_joins`. Each is independently shippable; the eligibility predicate is the single place that decides. The window sub-item above is the worked precedent.
